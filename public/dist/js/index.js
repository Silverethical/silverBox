...
async function renderTeamMembers(teamMembers) {
	...
	for (const teamMember of teamMembers) {
		...
		const { login, avatar_url, name } = await getSilverBoxTeamMembersInfo(
			githubUsername
		);
...
function copyConfig({ copy, iconElement, timeOut }) {
    ...
    if (iconElement) iconElement.classList.add('silverBox-copied');
    // changes it back to copy after 1s
    setTimeout(() => {
        if (iconElement) iconElement.classList.remove('silverBox-copied');
    }, timeOut);
}
...
document.addEventListener("DOMContentLoaded", () => {
	renderTeamMembers(silverBoxTeamMembers);
});
