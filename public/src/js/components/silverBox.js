...
export default function silverBox(config) {
	...
	if (config.inputs) {
		...
		if ("position" in config) {
			bodyEl.append(
				modalSample(elementsArray, `silver-box-${config.position}`)
			);
			silverBox = document.querySelector(".silver-box");
			silverBox.parentElement.classList.add(
				`silver-box-${config.position}`
			);
		} else {
	...
	if (
		!("confirmButtonCloseOnClick" in config) ||
		config.confirmButtonCloseOnClick === true
	) {
		if (confirmButton)
			confirmButton.addEventListener("click", closeOnClick);
	} else {
	...
	// deny close on click
	if (
		!("denyButtonCloseOnClick" in config) ||
		config.denyButtonCloseOnClick === true
	) {
	...
	if (
		!("cancelButtonCloseOnClick" in config) ||
		config.cancelButtonCloseOnClick === true
	) {
